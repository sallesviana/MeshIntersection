...
class MeshIntersectionGeometry {
	public:
		...
		~MeshIntersectionGeometry() {cerr << "Deletint memory..." << endl;}

		struct TempVarsIsCloser {VertCoord tempVertCoords[4];};
		bool isCloser(const InputVertex &origV, const Vertex &v1V, const Vertex &v2V, TempVarsIsCloser &tempVars) const;
		...
		struct TempVarsIsBoundaryClockwise {};
		bool isBoundaryInputTriangleClockwiselyOriented(const InputTriangle &t, TempVarsIsBoundaryClockwise &tempVars);
		...
		struct HeightPointInTriangleProjection {  VertCoord height; friend class MeshIntersectionGeometry;};
